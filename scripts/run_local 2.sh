...
if [ ! -f "frontend/.env" ]; then
  echo -e "${YELLOW}Creating default .env file for frontend...${NC}"
...
if check_dependency cargo && check_directory "backend/treasury_service"; then
...
if check_directory "frontend"; then
  ...
  echo -e "${RED}Error: frontend directory not found. Cannot start frontend server.${NC}"
  cleanup
fi
